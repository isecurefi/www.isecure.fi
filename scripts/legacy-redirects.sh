...
echo "== 1/8 ALB: remove the erroneous API-documentation redirect"
...
echo "== 2/8 ALB: reconcile the intended apex rules"
...
echo "== 3/8 ALB: require TLS 1.2 or newer"
aws elbv2 modify-listener --region "$REGION" \
  --listener-arn "$HTTPS_LISTENER_ARN" \
  --ssl-policy ELBSecurityPolicy-TLS13-1-2-Res-2021-06 >/dev/null

echo "== 4/8 CloudFront: update and publish the checked-in redirect function"
...
echo "== 5/8 CloudFront: attach redirects, modern TLS, and security headers"
CFG_RAW=$(mktemp)
CFG_JSON=$(mktemp)
...
echo "== 6/8 S3: back up and remove retired duplicate route objects"
mkdir -p "$BACKUP_DIR"
for key in \
...
echo "== 7/8 CloudFront: invalidate affected paths"
...
echo "== 8/8 Verify protected and redirected production routes"
