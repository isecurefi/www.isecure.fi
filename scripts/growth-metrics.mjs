...
export function renderGrowthMarkdown(report) {
  ...
  const lines = [
    ...
    "",
    ...(report.window.prior.startDate < ANALYTICS_METHOD_BOUNDARIES.at(-1).date
      ? [
          `GA4 method boundaries: ${ANALYTICS_METHOD_BOUNDARIES.map(({ date, method }) => `${method} starts ${date}`).join("; ")}. Do not interpret GA changes until both comparison windows start on or after the latest boundary.`,
          "",
        ]
      : []),
    "## Search discovery",
