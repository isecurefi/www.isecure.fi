...
- [C# / .NET 10 SDK (Experimental)](${CSHARP_SDK_URL}): registration, SMS/TOTP authentication, certificate enrollment and discovery, and signed file exchange. Follow the [C# quickstart](${CSHARP_SDK_URL}#tldr-make-your-first-api-call) to install from source; no NuGet release is published. See [preview coverage](${CSHARP_SDK_URL}/blob/main/docs/preview-scope.md) for the supported operations. Certificate import/export/sharing, password reset, and the separate Processing API are outside this preview.
- [PHP client and SDK (Legacy)](https://github.com/isecurefi/wscli-php): command-line client and PHP SDK. See its README for installation and usage.
